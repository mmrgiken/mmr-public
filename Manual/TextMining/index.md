...
# テキスト解析について
こちらでは、MMRのテキスト分析の大まかな流れをご紹介します。
まず、テキストデータを品詞分解により、単語データに分解します。
その後、単語データから「単語ランキング」「ワードクラウド」「ネットワーク図」といったグラフを生成します。
![](images/%E3%83%86%E3%82%AD%E3%82%B9%E3%83%88%E8%A7%A3%E6%9E%90%E3%81%AE%E6%B5%81%E3%82%8C.svg)
...
## ネットワークとは
ネットワークとは、ある２つの単語が１つの文章に両方入っていること（これを共起と呼びます）を単語のつながりと考えて、それをネットワーク図で表したものです。共起ネットワークとも呼ばれます。
![](images/2022-08-25-16-41-58.png)
...
## ネットワークのながれ
▼MMRでのネットワーク完成のイメージです。以下、おおまかな手順をご紹介します。
![](images/2022-09-09-11-51-56.png)

ネットワークは品詞分解が終わったところからスタートします。

1. ネットワークの種類の選択、単語の編集や、ネットワークに入れる単語の出現回数の最低水準を指定して、「ネットワーク図描画」を押します。
![](images/2022-09-09-12-04-10.png)

1. しばらくすると、ネットワークが表示されます。円の色は、単語つながりを元にした「コミュニティ」というまとまりを自動で計算して色分けされます。
あとは、用途に応じてネットワークの見た目を変更します。
![](images/2022-09-09-12-06-14.png)

1. 「大きさ・配置の設定」では、共起の計算方法やその他の「ネットワークの大きさや位置、表示方法」に関して指定できます。
![](images/2022-09-09-12-07-32.png)
例えば、ラベルを円の中に入れたり、円の繋がりはそのままで円の配置をランダムに変更したりできます。
また円をマウスのドラッグで個別に動かすこともできます。
![](images/2022-09-09-12-09-53.png)

1. 「色・コミュニティの設定」では円の色を変更できます。デフォルトでは前述の「コミュニティ」による自動色分けですが、指定の色にすることもできます。
![](images/2022-09-09-12-13-48.png)

## ネットワークの元文章
ネットワーク上の円（単語）をクリックして、「選択した項目の元データ表示」を押すと、その単語が含まれる元の文章が表示されます。Ctrlキーを押しながらクリックすることで複数選択も可能です。
![](images/2022-09-09-12-47-16.png)
![](images/2022-09-09-12-47-38.png)

## コミュニティ内文章ランキング
「コミュニティ内文章ランキング」のタブで、ネットワークのコミュニティごとに、そのコミュニティに属する単語を含む文章の中で「最も代表的な文章」のランキングを確認できます。
![](images/2022-09-09-12-49-12.png)

これは、元データのたくさんの文章をコミュニティに分け、その代表的な文章を抽出しているため、ざっくりと「データにどのような異なる傾向の文章があるか？」を把握するのに役立ちます。
※ただし、単語を多く含む文章は複数のコミュニティにまたがる場合もあるので注意が必要です。

コミュニティ内文章ランキングについては、順位の付け方などを以下で設定可能です。
![](images/2022-09-09-12-54-25.png)

## コミュニティ分類手法の手順
MMRにおける共起ネットワークでは、Louvain法を用いています。
詳しくは以下をご参照ください。
* [筑波大学講義資料(P12,P21)](https://www.ccs.tsukuba.ac.jp/wp-content/uploads/sites/14/2016/10//shiokawa.pdf)

* [原論文](documents/FastUnfoldingOfCommunitiesInLargeNetworks.pdf)


以下は簡単なLouvain法の説明になります。
##### コミュニティ分類の手順

1. 共起ネットワークグラフを作成する。

1. ネットワークの全てのノードについて、「線で繋がれたノード（隣接ノード）の中でモジュラリティが最も高くなるノード」を選び、同一コミュニティとする。
   （とりあえず手近な相手の中で最も適切な相手とくっつくイメージ）

2. 2で作ったコミュニティを、それぞれ1つのノードとして置き換える。
   （コミュニティ間に複数の線がある場合は、重み付きの1本の線と考える）

3. 2と3を繰り返す。
   「2において全ノードについてモジュラリティが向上する相手が誰も見つからない状態」となったら終了する。

##### Louvain法のイメージ
![](images/louvain_image.png)
※原論文より引用。赤文字はMMR開発チームで加筆したもの。

##### コミュニティ分類の指標：モジュラリティとは？
モジュラリティは、ネットワークをコミュニティに分割したときの、
「コミュニティ間の線の本数に対する、各コミュニティ内の線の本数」を表す指標です。
つまりこれが高いほど、「良い感じにネットワークを分類できている」と言えます。
※モジュラリティは「ネットワークの分け方」の評価指標なので、ネットワークに対して１つの値となります。

##### モジュラリティのイメージ
![](images/modularity_image.png)
※筑波大資料より引用。赤文字はMMR開発チームで加筆したもの。